...
//		jfile.setUseSecurity(fileVo.getUseSecurity());
	...
	public void upload(MultipartFile multipartFile, JFileDetails fileVo) {
		...
		fileVo.setFileMask(getFileMask(multipartFile.getOriginalFilename(), 0, fileVo.getUploadMode(), 
							JProperties.getString(GlovalVariables.DEFAULT_FILE_UPLOAD_PATH_KEY)));
		...
		String uploadDirectoryPath = getUploadDirectoryPath(
				JProperties.getString(GlovalVariables.DEFAULT_FILE_UPLOAD_PATH_KEY), fileVo.getUploadMode());
		
		String uploadPullPath = getUploadFilePullPath(fileVo.getFileMask(), fileVo.getUploadMode(), 
				(getUploadPathKey() != null && !"".equals(getUploadPathKey())) ? 
						JProperties.getString(getUploadPathKey()) : JProperties.getString(GlovalVariables.DEFAULT_FILE_UPLOAD_PATH_KEY));
		try {
			JFileUploadTypeFactory.INSTANCE.getUploadType(fileVo.getUseSecurity()).getHandler().handle(
					multipartFile.getInputStream(), new FileOutputStream(uploadPullPath));
